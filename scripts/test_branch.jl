...
function run_loops(loops, randomize)
    ...
    for i in 1:nloops
        branch_test!(loops[i], randomize, @view(res[:, i * 2 - 1]), @view(res[:, i * 2]))
    end
    return res
end
...
function run_all()
    dir_reg = joinpath(@__DIR__, "..", "data", "test_br_regular")
    dir_rand = joinpath(@__DIR__, "..", "data", "test_br_randomize")
